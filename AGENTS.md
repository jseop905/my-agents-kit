...
여러 AI 코딩 에이전트(Claude Code·Codex)용 설정을 **공통 원본 → 패키지** 모델로 관리하는 kit. 공통 원본은 이 repo가 들고, `bin/build`가 도구별 완성 패키지를 `dist/`에 생성한다. 프로젝트 설치·업데이트는 사용자가 dist를 직접 복사한다(도구는 kit 밖에 쓰지 않음).

## 구조

- `agent-kit.yml` — 무엇을 어디에 담는지 선언하는 manifest. **등록된 것만 패키지 포함.**
...
- `templates/*.hbs` — AGENTS.md·CLAUDE.md 생성 템플릿 (데이터 계약은 각 템플릿 상단 주석)
- `profiles/*.yml` — 프로젝트 유형별 preset (`profiles/README.md`)
- `bin/build.mjs` — 패키지 생성 스크립트 (무의존 — 내장 최소 YAML 파서·렌더러)
...
- **생성물 계약 동기화** — 템플릿 데이터 계약을 바꾸면 `templates/`와 `bin/build.mjs`를 함께 바꾸고, 생성 테스트로 확인한다.
- **동기화** — 자산을 추가/삭제/개명하면 `agent-kit.yml` 등록 목록과 `README.md`·`DESIGN.md` 상호 참조를 함께 갱신하고, `grep`으로 고아·깨진 참조를 확인한다.
- **커밋**은 `context/git.md` 원칙을, **테스트**는 `write-tests` 스킬을 따른다. `bin/` 변경은 `node --test bin/build.test.mjs`로 검증한다.
